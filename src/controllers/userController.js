...
export const registerUser = async (req, res) => {
    try {
      ...
      res.status(201).json(
        new ApiResponse(true, 201, "User registered successfully", user)
      );
    } catch (error) {
      console.error(error);
        ...
        ? {
            $match: {
              $or: [
                ...
                { _id: { $regex: search, $options: 'i' } }, // Search by stringified _id
  ...
  export const updateUserById = async (req, res) => {
    try {
      ...
      res.status(200).json(
        new ApiResponse(true, 200, "User updated successfully", user)
      );
    } catch (error) {
      console.error(error);
  ...
  export const deleteUserById = async (req, res) => {
    try {
      ...
      if (!existing) {
        return res
          .status(404)
          .json(new ApiResponse(false, 404, "No User Found"));
      }
